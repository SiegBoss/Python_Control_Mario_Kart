...
with pose.Pose(static_image_mode=False, model_complexity=0) as body:
    ...
    while True:
        ...
        if results.pose_landmarks is not None:
            ...
            drawing.draw_landmarks(frame, results.pose_landmarks, mp_holistic.POSE_CONNECTIONS,drawing.DrawingSpec(color=(128, 0, 255), thickness=2, circle_radius=1),drawing.DrawingSpec(color=(255, 255, 255), thickness=2))
            ...
            right_hand = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_INDEX]
            left_hand = results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_INDEX]

            #Convertir las coordenadas a pixeles | Convert coordinates to pixels 
            right_hand_px = drawing._normalized_to_pixel_coordinates(right_hand.x, right_hand.y, width, height)
            left_hand_px = drawing._normalized_to_pixel_coordinates(left_hand.x, left_hand.y, width, height)
            ...
            if right_hand_px and left_hand_px:
                ...
                cv2.line(frame, right_hand_px, left_hand_px, color=(255, 0, 0), thickness=2)
                ...
                dist = math.sqrt((left_hand_px [0] - centro_circulo[0])**2 + (left_hand_px [1] - centro_circulo[1])**2)
